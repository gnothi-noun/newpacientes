...
% En el preámbulo
% ---------- LINK insert ----------
\usepackage[hidelinks]{hyperref} % o sin hidelinks si querés que se vea azul
% ---------- Tablas ----------
\usepackage{booktabs}
\usepackage{array}
\usepackage{tabularx}
...
% \usepackage{tabu} % (no recomendado: suele romper; dejalo comentado)
...
\section{Introduccion}
    ...
    El problema que aborda este trabajo puede formularse, entonces, de la siguiente manera:
    \textit{¿Cómo diseñar e implementar una interfaz de usuario para la visualización de datos biométricos obtenidos mediante relojes inteligentes que resulte usable, comprensible y clínicamente útil para el equipo de salud de una residencia de personas mayores institucionalizadas?}
...
\section{Objetivos}
    ...
    \begin{enumerate}
        ...
        % \item \textbf{Transferencia y continuidad.} Dejar el sistema operativo y documentado, con una estructura modular que permita su mantenimiento y su continuidad por parte de terceros.
...
\section{Materiales y métodos}
    ...
    La información proviene de un volcado (\textit{dump}) de la base de datos MySQL/MariaDB de la Residencia Asturiana, generado con \texttt{mysqldump} y de un tamaño cercano a los 500 MB. Sobre ese volcado se trabajó de forma reproducible (el procesamiento se detalla en el Capítulo 6).

    El volcado contiene varias tablas, aunque solo dos concentran la información efectivamente utilizada. La Tabla~\ref{tab:tablas} resume su contenido.

    \begin{table}[h]
        \centering
        \begin{tabular}{l r l}
            \hline
            \textbf{Tabla} & \textbf{Filas} & \textbf{Observación} \\
            \hline
            ...
            \hline
        \end{tabular}
        \caption{Tablas presentes en el volcado de la Residencia Asturiana.}
        \label{tab:tablas}
    \end{table}

    El grueso del trabajo recae sobre \texttt{wearabledata}: más de 2,36 millones de registros que conforman las series temporales de seis métricas. La Tabla~\ref{tab:metricas} muestra su distribución. Las cinco variables fisiológicas tienen una cobertura muy pareja (cercana a los 468.000 registros cada una), mientras que los pasos diarios aparecen con una frecuencia mucho menor.

    \begin{table}[h]
        \centering
        \begin{tabular}{l r}
            \hline
            \textbf{Métrica} & \textbf{Registros} \\
            \hline
            Temperatura & 468.589 \\
    ...
    \begin{itemize}
        \item \textbf{Comorbilidades sin cargar.} Los campos de índice de Charlson, índice de Barthel y fecha de diálisis están vacíos, y el de diabetes figura en cero para todos los pacientes. No fue posible, por lo tanto, estratificar el análisis por comorbilidad.
        \item \textbf{Edad poco confiable.} Las fechas de nacimiento son valores de relleno (\textit{placeholders}), de modo que la edad calculada no es fiable y se trata con cautela.
        \item \textbf{Presión arterial sospechosa.} Los valores de presión aparecen acotados exactamente a los umbrales configurados, lo que sugiere que se trata de datos sintéticos. Por esa razón, la presión se visualiza pero se excluye del módulo de análisis de patrones.
    ...
    La herramienta se construyó íntegramente en Python, apoyándose en un conjunto acotado de librerías de código abierto. La Tabla~\ref{tab:stack} lista las principales, con las versiones efectivamente utilizadas y validadas.
...
\section{Implementación de la interfaz}
    ...
    % \begin{figure}[h]
    %     \centering
    %     \includegraphics[width=0.9\textwidth]{img/arquitectura_capas.png}
    ...
    El impacto de esta decisión se resume en la Tabla~\ref{tab:parquet}. La reducción de unos 520~MB a unos 10~MB en disco, y de varios gigabytes a unos 280~MB en memoria, es exactamente lo que hace posible correr el sistema completo —con sus 2.363.501 filas— sobre hardware de bajo costo.

    \begin{table}[h]
        \centering
        \begin{tabular}{l l l}
            \hline
            \textbf{Métrica} & \textbf{Formato intermedio} & \textbf{Parquet} \\
            \hline
    ...
    Todo lo que el equipo médico podría querer ajustar vive en un único archivo de configuración, separado de la lógica. Esto incluye, ante todo, la definición de las seis métricas: su nombre, su color, su unidad y sus rangos normales. La Tabla~\ref{tab:umbrales} los recoge.

    \begin{table}[h]
        \centering
        \begin{tabular}{l l r r}
            \hline
            \textbf{Métrica} & \textbf{Unidad} & \textbf{Mínimo} & \textbf{Máximo} \\
            \hline
    ...
    %     \caption{Dashboard de cohorte: panel de alertas y tabla resumen.}
    %     \label{fig:dashboard}
    % \end{figure}
    ...
    Cuando se llega a esta vista desde una alarma, el monitor no muestra el período por defecto sino una ventana de aproximadamente dos horas centrada en el evento, con el punto de la alarma marcado sobre el gráfico. Este comportamiento responde directamente a un requerimiento del equipo médico.

    % \begin{figure}[h]
    %     \centering
    %     \includegraphics[width=0.9\textwidth]{img/monitor.png}
    %     \caption{Monitor de paciente con métricas superpuestas y panel de estadísticas.}
    %     \label{fig:monitor}
    % \end{figure}
    ...
    % \begin{figure}[h]
    %     \centering
    %     \includegraphics[width=0.9\textwidth]{img/analisis.png}
    %     \caption{Sección de análisis: banda personal circadiana y tendencia semanal.}
    %     \label{fig:analisis}
    % \end{figure}
...
\section{Evaluación de usabilidad}
    ...
    \begin{itemize}
        ...
        \item Una \textbf{investigadora de la Universidad de Alcalá}, con un perfil más técnico, vinculada al proyecto desde la línea de investigación en teleasistencia.
    \end{itemize}
    ...
    Al finalizar, cada participante respondió el \textbf{cuestionario SUS} y cuatro preguntas abiertas (qué le resultó más útil, qué le resultó confuso, qué agregaría y si lo usaría en su trabajo diario).
...
\section{Resultados}
    ...
    El desarrollo alcanzó un sistema funcional, desplegado en producción y accesible de forma remota y segura. La Tabla~\ref{tab:funcionalidades} resume las funcionalidades implementadas.

    \begin{table}[h]
        \centering
        \begin{tabular}{l l}
            \hline
            \textbf{Funcionalidad} & \textbf{Detalle} \\
            \hline
    ...
    Ambas participantes completaron la totalidad de las tareas sin asistencia. La Tabla~\ref{tab:desempeno} muestra el detalle por tarea. En conjunto, sobre doce tareas resueltas se registraron únicamente dos desvíos menores, y la mayoría de las tareas se completaron en segundos.

    \begin{table}[h]
        \centering
        \begin{tabular}{c c c c c}
            \hline
            & \multicolumn{2}{c}{\textbf{Médica}} & \multicolumn{2}{c}{\textbf{Investigadora}} \\
            \textbf{Tarea} & Tiempo & Errores & Tiempo & Errores \\
            \hline
            1 & 0:02 & 0 & 0:16 & 0 \\
            2 & 0:46 & 0 & 2:00 & 1 \\
            3 & 0:20 & 0 & 1:18 & 0 \\
    ...
    La Tabla~\ref{tab:sus} presenta las respuestas a la adaptación de la escala SUS y el puntaje resultante para cada participante.

    \begin{table}[h]
        \centering
        \begin{tabular}{c l c c}
            \hline
            \textbf{Ítem} & \textbf{Afirmación (resumida)} & \textbf{Médica} & \textbf{Investig.} \\
            \hline
            ...
            \hline
            \multicolumn{2}{r}{\textbf{Puntaje SUS}} & \textbf{92,5} & \textbf{87,5} \\
            \hline
        \end{tabular}
...
% =====================================================================
%  references.bib  —  Bibliografía VITAICARE
%  Referencias verificadas para los Capítulos 3 (Marco teórico)
%  y 4 (Estado del arte). Compilar con BibTeX/biblatex.
% =====================================================================
